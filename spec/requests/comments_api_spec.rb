  ...
  describe "POST /recipes/{recipe_id}/comments" do
    ...
    context 'when you have logged in' do
      ...
      it 'can add comments to your own posts' do
        expect{
          post api_recipe_comments_path(recipe), params: comment_params
      ...
      it "can add comments to other users' posts" do
        ...
        expect{
          post api_recipe_comments_path(other_user_recipe), params: comment_params
        }.to change(Comment, :count).by 1
      end
    end
  ...
  describe "DELETE /recipes/{recipe_id}/comments #destroy" do
    ...
    context 'when you have logged in' do
      it 'can delete' do
        log_in user
        expect{
          delete api_recipe_comment_path(recipe, comment)
        }.to change(Comment, :count).by -1
      end
    end
    context 'when you have not logged in' do
    ...
    context "when you try to delete another user's comment" do
      ...
      it 'cannot delete' do
        expect{
          delete api_recipe_comment_path(recipe, @other_comment)
        }.to_not change(Comment, :count)
      end
    end
  end
